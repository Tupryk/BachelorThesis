...
def residual(data):
    ...
    for i in range(1, len(data['timestamp'])):
        ...
        a_vel = np.array([data['gyro.x'][i], data['gyro.y']
                         [i], data['gyro.z'][i]])*d2r
        prev_a_vel = a_vel = np.array(
            [data['gyro.x'][i-1], data['gyro.y'][i-1], data['gyro.z'][i-1]])*d2r
        acc = np.array([data['acc.x'][i], data['acc.y']
                       [i], data['acc.z'][i]-1])*g

        R = rowan.to_matrix(np.array([data['stateEstimate.qw'][i], data['stateEstimate.qx']
                            [i], data['stateEstimate.qy'][i], data['stateEstimate.qz'][i]]))
