...
for i, data_path in enumerate(data_paths):
    ...
    # origin = np.array([x[1:], y[1:]]).T
    # vector = np.array([f[:, 0], f[:, 1]]).T * 100
    # plt.quiver(origin[:,0], origin[:,1], vector[:,0], vector[:,1], angles='xy', scale_units='xy', scale=1, color='g', alpha=.3, label="Residual v2")

    f, _ = residual(data, use_rpm=True)
    origin = np.array([x[1:], y[1:]]).T
    vector = np.array([f[:, 0], f[:, 1]]).T*1e-2
    plt.quiver(origin[:,0], origin[:,1], vector[:,0], vector[:,1], angles='xy', scale_units='xy', scale=1, color='r', alpha=.1, label="Residual rpm")
